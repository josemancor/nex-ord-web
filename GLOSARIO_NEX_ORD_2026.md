...
Este documento es el fundamento teórico y algorítmico de la suite NEX_ORD. El enfoque NEX_ORD abandona la perspectiva patologizante tradicional. En su lugar, adoptamos un **enfoque ecosistémico**:
...
3. [🧮 III. Topología y Análisis Multidimensional (Suite NEX_ORD)](#-iii-topologia-y-analisis-multidimensional-suite-nex_ord)
...
## 🧮 III. TOPOLOGÍA Y ANÁLISIS MULTIDIMENSIONAL (Suite NEX_ORD)
...
*   📌 **SMIb (Sociomatriz Integrada Básica / Bipolar):** Estructura matricial fundamental en la suite NEX_ORD. Aglutina en una sola casilla toda la complejidad de la relación diádica: lo que el sujeto emite ($A_{2,ij}$), lo que recibe ($A_{3,ij}$) y las percepciones de ambos ($A_{1,ij}, A_{4,ij}$). **Incluye explícitamente en su notación el orden de elección jerárquico ($1^{\circ}, 2^{\circ}, 3^{\circ}, \dots$)** en el que cada miembro del grupo ha sido seleccionado.
    > ⚠️ **REGLA ARQUITECTÓNICA DE GENERACIÓN DE SMIb**: La matriz SMIb se genera **SIEMPRE a partir del Formato MORENO AMPLIADO a $E_{-k_1}$ y $R_{-k_2}$ y sus hipótesis selectivas automáticas**, NUNCA directamente desde BREVE. Cuando un estudio se recaba de campo en formato BREVE, el motor NEX_ORD convierte primero la matriz continua de ordenamiento al formato MORENO ampliado (imputando los polos $E_{-k_1}, R_{-k_2}$ y los umbrales de meta-percepción $aE, oE$), y desde esta base de MORENO ampliado se compila la estructura final SMIb de 4 vías.
*   📌 **Densidad Relacional Total ($SDR_i$):** Índice matemático estandarizado computado por NEX_ORD como la tensión estructural neta normalizada:
...
*   📌 **CAJ (Clasificación Automática Jerarquizada):** Técnica estadística de clasificación jerárquica y agrupamiento (clustering) utilizada por NEX_ORD para fusionar iterativamente a los individuos topológicamente más próximos, delimitando fronteras y coaliciones (Benzécri, 1973; Roux, 1985; Ward, 1963).
...
El sistema NEX_ORD soporta dos formatos de recabado de datos sociométricos de campo:
...
Para recuperar las 4 vías ($A_1 \dots A_4$) en el formato BREVE sin añadir preguntas adicionales, NEX_ORD utiliza dos **umbrales sintéticos de frontera**:
...
Cuando el tamaño del colectivo es muy numeroso ($N > 20$), exigir un ordenamiento continuo directo de una sola pasada resulta inviable. NEX_ORD resuelve la ordenación total en BREVE aplicando un **protocolo algorítmico estructurado en 5 Fases consecutivas (Trabajar los Extremos)**:
...
Como derivación directa del modelo de Sociometría Ordinal Computacional, NEX_ORD establece las funciones biyectivas de transformación entre ambos formatos:
...
El motor NEX_ORD ha sustituido el simple "recuento de preferencias nominales" para medir la Energía Estructural de los vínculos:
...
Esta tabla agrupa la notación formal de NEX_ORD. Cada símbolo reúne su definición formal, su dominio numérico estricto y su interpretación ecosistémica.
...
Para ilustrar de forma concreta la notación y trazabilidad de NEX_ORD con total **anonimato nominal**, consideremos un grupo de **$N=4$ miembros censados**: **$S_1$**, **$S_2$**, **$S_3$** y **$S_4$**, evaluados en las 4 vías ($A_1, A_2, A_3, A_4$), 10 Variables Socio-Demográficas ($VAR$) y 16 Adjetivos Bipolares ($AAG$).
...
Las siguientes referencias bibliográficas constituyen el corpus teórico, estadístico y metodológico que fundamenta las definiciones y algoritmos de **NEX_ORD**:
